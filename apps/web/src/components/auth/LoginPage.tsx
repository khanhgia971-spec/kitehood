  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setErr((e && e.message) || "OAuth error");
      }
    })();
    ...
    <div className="min-h-screen bg-[#0b0f19] text-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-md p-6 rounded-2xl border border-white/10 bg-white/5">
          <h1 className="text-xl font-bold mb-4">{mode === "login" ? "Dang nhap" : "Tao tai khoan"}</h1>
          <a href="/api/auth/google" className="flex items-center justify-center gap-2 w-full mb-2 py-2.5 rounded-xl bg-white text-black font-semibold text-sm">
            <GoogleIcon /> Dang nhap voi Google
          </a>
          <a href="/api/auth/github" className="flex items-center justify-center gap-2 w-full mb-5 py-2.5 rounded-xl bg-[#24292f] font-semibold text-sm border border-white/10">
          ...
          <button type="button" className="mt-4 w-full text-sm text-indigo-300" onClick={() => setMode(mode === "login" ? "register" : "login")}>
            {mode === "login" ? "Chua co TK? Tao tai khoan" : "Da co TK? Dang nhap"}
          </button>
        </div>
      </div>
    </div>
