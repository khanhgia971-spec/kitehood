...
async function handleApi(request: Request, env: Env): Promise<Response> {
  ...
  if (request.method === 'POST' && path === '/auth/register') {
    ...
    if (await env.KV.get(`user:email:${email}`)) return json({ error: 'Email đã được sử dụng' }, 409);
  ...
  if (request.method === 'POST' && path === '/auth/login') {
    ...
    if (!user.password_hash) {
      return json({ error: 'Tài khoản này đăng nhập bằng Google/GitHub' }, 401);
    }
    const password_hash = await hashPassword(body.password, secret);
    if (password_hash !== user.password_hash) return json({ error: 'Email hoặc mật khẩu sai' }, 401);
    if (user.banned || user.is_banned) {
      return json({
